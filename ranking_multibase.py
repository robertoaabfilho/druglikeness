...
# =========================================================================
# PARTE 6b — Ranking por TOPSIS híbrido (ideal = perfil das drogas de
# referência, mas a matemática segue o formalismo padrão do TOPSIS)
# =========================================================================
#
# Diferença central para o método 'perfil' (pontuar_composto/gerar_ranking):
#   - 'perfil': similaridade por coluna (kernel gaussiano / match) e soma
#     ponderada simples -- não é TOPSIS, é um score de similaridade.
#   - 'topsis': monta uma matriz de decisão real (compostos x critérios),
#     normaliza pela norma vetorial de cada coluna, aplica os pesos, e
#     calcula a distância euclidiana até uma Solução Ideal Positiva (A+)
#     e uma Solução Ideal Negativa (A-), retornando o coeficiente de
#     proximidade relativa C_i = D-/(D+ + D-) -- a fórmula clássica do
#     TOPSIS. A parte "híbrida" é que A+ não vem do máximo/mínimo do
#     próprio pool de compostos, e sim do PERFIL DAS DROGAS DE REFERÊNCIA
#     (a média, para colunas numéricas; a moda, para booleanas/categóricas)
#     -- injetado na matriz como uma linha-alvo adicional antes de
#     normalizar, para que ela fique na mesma escala normalizada que os
#     compostos reais. A- é, para cada critério, o valor mais distante do
#     alvo entre os observados nos compostos (o "pior caso" em relação ao
#     perfil-alvo, não um mínimo/máximo genérico).

def _valor_alvo_topsis(info: dict) -> float:
    """O valor-alvo (linha 'ideal') de uma característica, na mesma escala
    numérica usada na matriz de decisão."""
    if info["tipo"] == "numerico":
        return float(info["media"])
    if info["tipo"] == "booleano":
        return 1.0 if info["moda"] else 0.0
    return 1.0  # categórico: a categoria-alvo bate consigo mesma por definição


def _codificar_valor_topsis(valor_bruto, info: dict) -> float | None:
    """
    Converte o valor bruto de uma célula do farmaco.csv para a escala
    numérica da matriz de decisão. Numéricas mantêm o valor real; booleanas
    viram 1.0/0.0 (estado real, não um indicador de 'bateu ou não bateu' --
    isso preserva informação genuína para a distância euclidiana); e
    categóricas (nominais, sem ordem natural) viram 1.0 se a categoria do
    composto bate com a categoria mais comum de referência, senão 0.0.
    """
    if info["tipo"] == "numerico":
        return _para_float(valor_bruto)
    if info["tipo"] == "booleano":
        valor = _para_bool(valor_bruto)
        return None if valor is None else (1.0 if valor else 0.0)
    valor = _categoria_de(valor_bruto)
    if not valor:
        return None
    return 1.0 if valor == info["moda"] else 0.0


def montar_matriz_topsis(linhas_farmaco: list[dict], pesos: dict[str, dict]):
    """
    Monta a matriz de decisão (compostos x critérios) e a linha-alvo
    (perfil das drogas de referência), na mesma ordem de colunas.
    Valores ausentes são imputados com o próprio valor-alvo daquela coluna
    (uma ausência não deveria, por si só, penalizar nem favorecer o
    composto -- fica neutro em relação ao ideal).
    Retorna (matriz, linha_alvo, colunas, nomes_smiles, faltantes_por_coluna).
    """
    colunas = list(pesos.keys())
    linha_alvo = [_valor_alvo_topsis(pesos[c]) for c in colunas]

    matriz = []
    nomes_smiles = []
    faltantes_por_coluna = {c: 0 for c in colunas}

    for linha in linhas_farmaco:
        vetor = []
        for c in colunas:
            info = pesos[c]
            valor = _codificar_valor_topsis(linha.get(c), info)
            if valor is None:
                valor = _valor_alvo_topsis(info)
                faltantes_por_coluna[c] += 1
            vetor.append(valor)
        matriz.append(vetor)
        nomes_smiles.append((linha.get("Nome", ""), linha.get("SMILES", "")))

    return (
        np.array(matriz, dtype=float),
        np.array(linha_alvo, dtype=float),
        colunas,
        nomes_smiles,
...
def normalizar_e_ponderar_topsis(matriz: np.ndarray, linha_alvo: np.ndarray, pesos: dict[str, dict], colunas: list[str]):
    """
    Normalização vetorial padrão do TOPSIS: r_ij = x_ij / sqrt(soma dos
    quadrados da coluna j) -- calculada sobre a matriz completa (compostos
    + linha-alvo), para que a linha-alvo fique na MESMA escala normalizada
    dos compostos reais. Depois aplica os pesos: v_ij = peso_j * r_ij.
    Retorna (V_matriz, V_alvo).
    """
    completa = np.vstack([matriz, linha_alvo.reshape(1, -1)])
    normas = np.sqrt((completa ** 2).sum(axis=0))
    normas[normas == 0] = 1.0  # evita divisão por zero se uma coluna inteira for 0
    r_completa = completa / normas

    vetor_pesos = np.array([pesos[c]["peso"] for c in colunas], dtype=float)
    v_completa = r_completa * vetor_pesos

    v_matriz = v_completa[:-1, :]
    v_alvo = v_completa[-1, :]
    return v_matriz, v_alvo


def calcular_ideal_negativo_topsis(v_matriz: np.ndarray, v_alvo: np.ndarray) -> np.ndarray:
    """
    Para cada critério, a Solução Ideal Negativa é o valor OBSERVADO nos
    compostos mais distante da linha-alvo (não um mínimo/máximo genérico
    de benefício/custo -- aqui 'pior' significa 'mais longe do perfil das
    drogas de referência', em qualquer direção).
    """
    if v_matriz.shape[0] == 0:
        return v_alvo.copy()
    maximos = v_matriz.max(axis=0)
    minimos = v_matriz.min(axis=0)
    mais_longe_do_max = np.abs(maximos - v_alvo)
    mais_longe_do_min = np.abs(minimos - v_alvo)
    return np.where(mais_longe_do_max >= mais_longe_do_min, maximos, minimos)


def gerar_ranking_topsis(linhas_farmaco: list[dict], pesos: dict[str, dict]) -> tuple[list[dict], dict[str, int]]:
    """
    Executa o TOPSIS híbrido completo e retorna (ranking, faltantes_por_coluna).
    Cada item do ranking traz: Nome, SMILES, Pontuacao_Final (= C_i * 100,
    a proximidade relativa ao ideal, de 0 a 100), D_mais (distância euclidiana
    até a Solução Ideal Positiva) e D_menos (distância até a Negativa).
    """
    matriz, linha_alvo, colunas, nomes_smiles, faltantes = montar_matriz_topsis(linhas_farmaco, pesos)

    if matriz.size == 0:
        return [], faltantes

    v_matriz, v_alvo = normalizar_e_ponderar_topsis(matriz, linha_alvo, pesos, colunas)
    v_negativo = calcular_ideal_negativo_topsis(v_matriz, v_alvo)

    d_mais = np.sqrt(((v_matriz - v_alvo) ** 2).sum(axis=1))
    d_menos = np.sqrt(((v_matriz - v_negativo) ** 2).sum(axis=1))
    soma = d_mais + d_menos
    proximidade = np.divide(d_menos, soma, out=np.zeros_like(d_menos), where=soma != 0)

    ranking = []
    for i, (nome, smiles) in enumerate(nomes_smiles):
        ranking.append({
            "Nome": nome,
            "SMILES": smiles,
            "Pontuacao_Final": round(float(proximidade[i]) * 100, 2),
            "D_mais": round(float(d_mais[i]), 4),
            "D_menos": round(float(d_menos[i]), 4),
        })
...
def salvar_ranking(ranking: list[dict], caminho_saida: str, delimitador: str = CSV_DELIMITADOR_PADRAO) -> None:
    # Inclui D_mais/D_menos quando presentes (saída do método TOPSIS),
    # mantendo compatibilidade com o método 'perfil' (que não os tem).
    colunas = ["Ranking", "Nome", "SMILES", "Pontuacao_Final"]
    if ranking and "D_mais" in ranking[0]:
        colunas += ["D_mais", "D_menos"]
...
def main():
    ...
    parser.add_argument(
        "--metodo", default="perfil", choices=["perfil", "topsis"],
        help=(
            "'perfil' (padrão): pontua cada composto pela semelhança à MÉDIA das drogas de "
            "referência (soma ponderada de similaridades, 0-100). 'topsis': TOPSIS híbrido -- "
            "segue o formalismo padrão do TOPSIS (matriz de decisão, normalização vetorial, "
            "distância euclidiana até uma Solução Ideal), mas o 'ideal' continua sendo o "
            "PERFIL DAS DROGAS DE REFERÊNCIA (não o próprio farmaco.csv)."
        ),
    )
    ...
    if args.metodo == "topsis":
        ranking, faltantes_topsis = gerar_ranking_topsis(linhas_farmaco, pesos)
        for coluna, n in faltantes_topsis.items():
            if n:
                print(f"[INFO][TOPSIS] Coluna '{coluna}': {n} composto(s) sem esse dado, imputado(s) com o valor-alvo.")
    else:
