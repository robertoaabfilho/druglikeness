...
-------------------------------------------------------------------
  ...
  6. mods/checar_alertas_adversos.py  -- checa bula por efeito adverso (rodar_alertas_adversos)
     Verifica se algum composto do ranking tem o(s) termo(s) de alerta
     (ex: "neuropathy") em warnings/adverse_reactions/boxed_warning da
     bula (openFDA) -- nunca nas indicações -- e gera um ranking ajustado
     com penalidade para os sinalizados.
...
# Termo(s) de alerta a buscar em warnings/adverse_reactions/boxed_warning das
# bulas (openFDA) -- nunca nas indicações. Ex: se o sintoma é dor neuropática,
# um composto famoso por CAUSAR neuropatia (efeito adverso) é sinalizado aqui.
termos_alerta = neuropathy
...
def main():
    ...
    rodar_alertas = get_bool(config, "rodar_alertas_adversos", False)
    ...
    # ---- Etapa 6 (opcional): mods/checar_alertas_adversos.py ----
    if rodar_alertas:
        if not verificar_e_instalar_dependencias(DEPENDENCIAS_OBRIGATORIAS["alertas"]):
            print("[AVISO] Dependências da etapa checar_alertas_adversos.py não puderam ser instaladas. Etapa pulada.")
        else:
            termos_alerta = get_str(config, "termos_alerta", "neuropathy")
            penalidade_alerta = get_float(config, "penalidade_alerta", 0.5)
            comando = [
                python_exe, str(MODS_DIR / "checar_alertas_adversos.py"),
                "--ranking", ranking_output,
                "--termos", termos_alerta,
                "--penalidade", str(penalidade_alerta),
                "--output", str(results_dir / "ranking.alertas.csv"),
            ]
            if not rodar_etapa("checar_alertas_adversos.py (checagem de efeito adverso na bula)", comando):
                print("[AVISO] Etapa de checagem de alertas falhou, mas o pipeline principal já tinha concluído -- não é um erro fatal.")
