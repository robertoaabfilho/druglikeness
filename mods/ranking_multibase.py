...
SIMULTANEAMENTE para achar drogas indicadas para um sintoma, mescla os
...
USO
---
    python ranking_multibase.py --sintoma "Alzheimer's disease" \
        --farmaco farmaco.csv --output ranking.csv

    # Desativar alguma fonte:
    python ranking_multibase.py --sintoma Alzheimer --no-drugcentral --no-openfda

    # Sem consultar ADMET das drogas de referência (mais rápido):
    python ranking_multibase.py --sintoma Alzheimer --no-admet
...
from configuracao import carregar_config, get_str, get_int, get_list
...
def revisar_referencias_interativamente(nomes_smiles_fontes: list[tuple[str, str, set]]) -> list[tuple[str, str, set]]:
    """
    Mostra a lista de drogas de referência mescladas e pergunta ao usuário
    se ele quer remover alguma antes de prosseguir (cálculo de pesos,
    ADMET, ranking). Retorna a lista filtrada.

    Se a entrada padrão não for interativa (ex: rodando via pipeline
    automatizado sem terminal), qualquer EOFError é tratada como "manter
    todas" -- não trava a execução.
    """
    print("\n" + "=" * 60)
    print("DROGAS DE REFERÊNCIA ENCONTRADAS")
    print("=" * 60)
    for i, (nome, _smiles, fontes) in enumerate(nomes_smiles_fontes, start=1):
        print(f"  [{i:>2}] {nome:<35s} (fontes: {'+'.join(sorted(fontes))})")

    try:
        resposta = input(
            "\nDigite os números das drogas a REMOVER, separados por vírgula "
            "(ou Enter para manter todas): "
        ).strip()
    except EOFError:
        print("[AVISO] Entrada não interativa detectada -- mantendo todas as drogas de referência.")
        return nomes_smiles_fontes

    if not resposta:
        print("[INFO] Nenhuma droga removida. Mantendo todas.")
        return nomes_smiles_fontes

    try:
        indices_remover = {int(p.strip()) for p in resposta.split(",") if p.strip()}
    except ValueError:
        print("[AVISO] Entrada inválida (esperado números separados por vírgula). Mantendo todas as drogas.")
        return nomes_smiles_fontes

    indices_validos = {i for i in indices_remover if 1 <= i <= len(nomes_smiles_fontes)}
    invalidos = indices_remover - indices_validos
    if invalidos:
        print(f"[AVISO] Número(s) fora do intervalo, ignorado(s): {sorted(invalidos)}")

    filtrada = [item for i, item in enumerate(nomes_smiles_fontes, start=1) if i not in indices_validos]
    removidas = [item[0] for i, item in enumerate(nomes_smiles_fontes, start=1) if i in indices_validos]

    if removidas:
        print(f"[INFO] Removida(s): {', '.join(removidas)}")
    print(f"[INFO] {len(filtrada)} droga(s) de referência mantida(s) para o restante do pipeline.")
    return filtrada
...
def main():
    ...
    parser.add_argument("--sintoma", nargs="+", default=None, help="Palavra(s)-chave do sintoma/condição (em inglês). Pode vir do config.txt (chave 'sintomas') se não for passado aqui.")
    parser.add_argument("--farmaco", default=None, help="Caminho do farmaco.csv de entrada. Padrão: farmaco.csv, ou 'farmaco_output'/'ranking_farmaco' do config.txt.")
    parser.add_argument("--output", default=None, help="Caminho do CSV de ranking de saída. Padrão: ranking.csv, ou 'ranking_output' do config.txt.")
    ...
    parser.add_argument("--top-por-fonte", type=int, default=None, help="Quantas drogas manter de CADA fonte (Open Targets, ChEMBL, DrugCentral), priorizando maior fase clínica/aprovação. Padrão: 10, ou 'top_por_fonte' do config.txt.")
    ...
    parser.add_argument("--pred-type", default=None, choices=["absorption", "distribution", "metabolism", "excretion", "admet"], help="Tipo de predição do Deep-PK. Padrão: admet, ou 'pred_type' do config.txt.")
    parser.add_argument("--pular-revisao-referencias", action="store_true", help="Não pergunta interativamente se quer remover drogas de referência antes de prosseguir (útil para automação/pipeline).")
    parser.add_argument("--config", default="config.txt", help="Caminho do config.txt (padrão: config.txt; opcional).")
    args = parser.parse_args()

    config = carregar_config(args.config)
    if args.sintoma is None:
        args.sintoma = get_list(config, "sintomas") or None
    if not args.sintoma:
        print("[ERRO] Nenhum sintoma informado (nem em --sintoma, nem em 'sintomas' no config.txt).", file=sys.stderr)
        sys.exit(1)
    if args.farmaco is None:
        args.farmaco = get_str(config, "farmaco_output", get_str(config, "ranking_farmaco", "farmaco.csv"))
    if args.output is None:
        args.output = get_str(config, "ranking_output", "ranking.csv")
    if args.top_por_fonte is None:
        args.top_por_fonte = get_int(config, "top_por_fonte", 10)
    if args.pred_type is None:
        args.pred_type = get_str(config, "pred_type", "admet")
    if not args.no_admet:
        args.no_admet = get_str(config, "no_admet", "false").strip().lower() in ("1", "true", "sim", "yes", "on")
    if not args.pular_revisao_referencias:
        args.pular_revisao_referencias = get_str(config, "pular_revisao_referencias", "false").strip().lower() in ("1", "true", "sim", "yes", "on")
    ...
    resultado_ot, efo_ids = ({}, []) if args.no_opentargets else buscar_open_targets(args.sintoma, args.top_por_fonte)
    resultado_chembl = {} if args.no_chembl else buscar_chembl_indicacao(args.sintoma, args.top_por_fonte, efo_ids=efo_ids)
    resultado_dc = {} if args.no_drugcentral else buscar_drugcentral(args.sintoma, args.top_por_fonte)
    resultado_fda = {} if args.no_openfda else buscar_openfda(args.sintoma, args.top_por_fonte)
    ...
    # ---- Revisão interativa: usuário pode remover drogas antes de prosseguir ----
    if not args.pular_revisao_referencias:
        nomes_smiles_fontes = revisar_referencias_interativamente(nomes_smiles_fontes)
        if not nomes_smiles_fontes:
            print("[ERRO] Todas as drogas de referência foram removidas. Encerrando.", file=sys.stderr)
            sys.exit(1)
