...
Une o cálculo de descritores de drogabilidade (druglikeness_rdkit.py) com a
consulta à API ADMET do Deep-PK (deeppk_query.py) em um único fluxo:
...
Uso:
    python farmaco_completo.py [--input input.txt] [--output farmaco.csv]
                                [--pred-type admet] [--batch-size 5]

Requisitos:
    pip install rdkit requests

Este módulo também é projetado para ser IMPORTADO por outros scripts (por
exemplo, ranking_multibase.py), que reaproveitam diretamente as funções
`calcular_descritores`, `avaliar_regra_dos_5`, `avaliar_regra_de_veber` e,
principalmente, `processar_lista_druglikeness`, em vez de duplicar essa
lógica.
"""
...
def submeter_job(smiles_lote: list[str], pred_type: str, email: str | None) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            f"Não foi possível decodificar a resposta da API ao submeter o job: {erro}\n"
            f"Resposta bruta recebida: {resposta.text[:500]!r}"
        )

    job_id = payload.get("job_id") if isinstance(payload, dict) else None
    if not job_id:
        raise RuntimeError(
            f"Resposta inesperada da API ao submeter o job: {payload}\n"
            f"SMILES enviado(s) neste lote: {smiles_lote}"
        )
...
def aguardar_resultado(job_id: str) -> dict:
    """
    Consulta periodicamente o status do job até que o processamento termine,
    retornando o dicionário de resultados.
    """
    inicio = time.time()
    ...
    while True:
        ...
        try:
            ...
            raise RuntimeError(
                f"Não foi possível decodificar a resposta da API para o job {job_id}: {erro}\n"
                f"Resposta bruta recebida: {resposta.text[:500]!r}"
            )
        ...
        if isinstance(payload, dict) and payload.get("status") == "running":
            ...
            if decorrido > MAX_WAIT_SECONDS:
                raise TimeoutError(
                    f"Tempo máximo de espera ({MAX_WAIT_SECONDS}s) excedido para o job {job_id}."
                )
        ...
        if not isinstance(payload, dict):
            raise RuntimeError(
                f"Resposta inesperada da API para o job {job_id} (esperava um dicionário, "
                f"recebi {type(payload).__name__}): {str(payload)[:500]!r}"
            )
...
def resultados_para_linhas(resultados: dict, indice_inicial_global: int, colunas_parametros: list[str]) -> list[dict]:
    """
    Converte o dicionário de resultados de um lote em uma lista de linhas,
    atribuindo o índice global original (posição no input.txt) a cada
    molécula — essencial para juntar depois com os descritores de
    drogabilidade.
    """
    linhas = []
    ...
    for indice_local in indices_moleculas:
        ...
        linha = {
            "indice_original": indice_inicial_global + int(indice_local),
...
def consultar_deeppk(
    ...
    """
    Consulta a API do Deep-PK para toda a smiles_list, em lotes, salvando
    incrementalmente em 'caminho_raw' (CSV intermediário) com checkpoint em
    'caminho_checkpoint'. Retorna (colunas_parametros, caminho_raw).
    """
    total = len(smiles_list)

    proximo_indice = carregar_checkpoint(caminho_checkpoint)
    if proximo_indice > 0:
        print(f"[INFO] Checkpoint do Deep-PK encontrado. Retomando a partir do SMILES nº {proximo_indice + 1} de {total}.")
    ...
    while indice_atual < total:
        lote = smiles_list[indice_atual: indice_atual + batch_size]
        n_lote = len(lote)
        print(f"[INFO] Consultando Deep-PK para SMILES {indice_atual + 1}\u2013{indice_atual + n_lote} de {total}...")

        try:
            job_id = submeter_job(lote, pred_type, email)
            print(f"[INFO]   job_id = {job_id}")
            resultados = aguardar_resultado(job_id)
        except Exception as erro_lote:
            # O lote inteiro falhou (frequentemente por causa de UMA molécula
            # "envenenada" no meio dele). Em vez de travar o script inteiro
            # (e ficar preso nesse mesmo lote a cada nova tentativa), tenta
            # cada SMILES individualmente e pula só o(s) que realmente falhar(em).
            print(f"[AVISO]   Falha ao processar o lote inteiro: {erro_lote}")
            print(f"[AVISO]   Tentando SMILES individualmente para isolar o problema...")
            resultados = {}
            for offset_local, smiles_individual in enumerate(lote):
                try:
                    job_id_ind = submeter_job([smiles_individual], pred_type, email)
                    resultado_ind = aguardar_resultado(job_id_ind)
                    dados_mol = resultado_ind.get("0")
                    if not isinstance(dados_mol, dict):
                        raise RuntimeError(f"Resposta sem dados válidos: {resultado_ind}")
                    resultados[str(offset_local)] = dados_mol
                    print(f"[INFO]     SMILES {indice_atual + offset_local + 1} OK.")
                except Exception as erro_individual:
                    print(
                        f"[AVISO]     SMILES {indice_atual + offset_local + 1} rejeitado pelo Deep-PK "
                        f"e será IGNORADO (sem dados ADMET, mas continua no farmaco.csv): "
                        f"'{smiles_individual}' -> {erro_individual}"
                    )

        if colunas_parametros is None and resultados:
            colunas_parametros = extrair_colunas(resultados)
            print(f"[INFO]   {len(colunas_parametros)} colunas de par\u00e2metros ADMET detectadas.")

        if colunas_parametros:
            linhas = resultados_para_linhas(resultados, indice_atual, colunas_parametros)
            colunas_finais = construir_colunas_deeppk(colunas_parametros)
            anexar_csv(linhas, caminho_raw, colunas_finais)
        elif resultados:
            print(
                "[AVISO]   Nenhuma coluna de parâmetros pôde ser determinada ainda "
                "(primeiro lote sem nenhum sucesso) — estes resultados serão perdidos."
            )

        # Sempre avança o checkpoint, mesmo que parte do lote tenha falhado —
        # é isso que evita ficar preso para sempre no mesmo SMILES problemático.
        indice_atual += n_lote
        salvar_checkpoint(caminho_checkpoint, indice_atual)

        print(f"[INFO]   Progresso Deep-PK: {indice_atual}/{total}.")

    print(f"[INFO] Consulta ao Deep-PK concluída para os {total} SMILES.")
    if os.path.exists(caminho_checkpoint):
        os.remove(caminho_checkpoint)

    return colunas_parametros or [], caminho_raw
...
def main():
    ...
    parser.add_argument("--input", default=None, help="Arquivo de entrada com os SMILES (um por linha). Padrão: input.txt, ou 'farmaco_input' do config.txt.")
    parser.add_argument("--output", default=None, help="Arquivo CSV final de saída. Padrão: farmaco.csv, ou 'farmaco_output' do config.txt.")
    parser.add_argument(
        "--raw-deeppk",
        default=None,
    ...
    parser.add_argument(
        ...
        help="Tipo de predição do Deep-PK a ser realizada. Padrão: admet, ou 'pred_type' do config.txt.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=None,
        help="Quantos SMILES enviar por job ao Deep-PK / salvar por vez. Padrão: 5, ou 'batch_size' do config.txt.",
    )
    parser.add_argument("--email", default=None, help="E-mail opcional para notificação ao final do job do Deep-PK.")
    parser.add_argument("--config", default="config.txt", help="Caminho do config.txt (padrão: config.txt; opcional -- se não existir, usa só os argumentos de linha de comando).")
    args = parser.parse_args()

    config = carregar_config(args.config)
    if args.input is None:
        args.input = get_str(config, "farmaco_input", "input.txt")
    if args.output is None:
        args.output = get_str(config, "farmaco_output", "farmaco.csv")
    if args.pred_type is None:
        args.pred_type = get_str(config, "pred_type", "admet")
    if args.batch_size is None:
        args.batch_size = get_int(config, "batch_size", 5)
    ...
    try:
        ...
        smiles_list = [smiles for _, smiles in moleculas]
        ...
        # ---- Etapa 3: combinar e salvar em farmaco.csv ----
        combinar_e_salvar(
