...
SMILES com múltiplos fragmentos desconectados ('.', ex.: sais como
'CCCC(CCC)C(=O)[O-].[Na+]') são padronizados automaticamente — reduzidos ao
maior fragmento e neutralizados — antes de qualquer cálculo, tanto local
(RDKit) quanto na consulta ao Deep-PK (que rejeita SMILES com '.'). O
SMILES original é preservado na coluna 'SMILES' do CSV final; o que foi
efetivamente analisado fica em 'SMILES_Usado'. Ver padronizar_smiles() e o
comentário na PARTE 0 abaixo para a ressalva sobre contra-íons metálicos.
...
# =========================================================================
# PARTE 0 — padronização automática de SMILES com múltiplos fragmentos ('.')
# =========================================================================
#
# Tanto o RDKit (para alguns descritores) quanto, principalmente, a API do
# Deep-PK rejeitam SMILES com mais de um fragmento desconectado (sais,
# pares iônicos etc. representados com '.', ex.: 'CCCC(CCC)C(=O)[O-].[Na+]').
#
# Sempre que um SMILES contiver '.', ele é reduzido automaticamente ao seu
# "parent": o maior fragmento (heurística padrão de pipelines de
# padronização como ChEMBL/MolVS), com as cargas remanescentes neutralizadas
# (ex.: carboxilato -> ácido carboxílico). É esse SMILES padronizado que é
# usado tanto no cálculo dos descritores quanto no envio ao Deep-PK — o
# SMILES original de entrada é sempre preservado à parte, para rastreio.
#
# Ressalva importante: "maior fragmento" é uma convenção mecânica, não uma
# avaliação farmacológica. Ela funciona bem quando o contra-íon é inerte
# (Na+, K+ em sais de ácidos/bases orgânicos — ex.: valproato de sódio).
# Mas quando o fragmento descartado contém um metal fora dessa lista
# "habitual" (Zn, Li, Fe, Ca, Mg, Cu, Bi, Pt...), ele pode ser a própria
# espécie farmacologicamente ativa (ex.: Zn2+ em acetato de zinco) — nesse
# caso a predição de PK feita sobre o fragmento orgânico remanescente não
# tem sentido clínico. Por isso essas linhas são sinalizadas em
# 'Aviso_Padronizacao' para checagem manual, em vez de processadas em
# silêncio.

SIMBOLOS_METALICOS = {
    "Li", "Na", "K", "Rb", "Cs",
    "Mg", "Ca", "Sr", "Ba",
    "Zn", "Fe", "Cu", "Mn", "Co", "Ni", "Al", "Ag", "Au", "Pt", "Pd",
    "Bi", "Sn", "Pb", "Cr", "Cd", "Hg", "Ti", "V", "Mo", "Se", "Ga",
}
# Contra-íons metálicos comuns o suficiente em sais orgânicos (ex.: sódio/
# potássio de ácidos carboxílicos) para NÃO gerar aviso — a remoção deles é
# a prática padrão em pipelines de ADMET/PK.
METAIS_CONTRAION_HABITUAL = {"Na", "K"}

_LARGEST_FRAGMENT_CHOOSER = rdMolStandardize.LargestFragmentChooser()
_UNCHARGER = rdMolStandardize.Uncharger()


def padronizar_smiles(smiles: str) -> dict:
    """
    Se 'smiles' tiver múltiplos fragmentos desconectados (contém '.'),
    reduz ao maior fragmento e neutraliza cargas remanescentes. Se não
    houver '.', devolve o próprio SMILES original sem alterações.

    Retorna um dicionário com:
        smiles_usado           -- SMILES a usar nos cálculos/consulta
        multi_fragmento        -- True se o SMILES original tinha '.'
        contra_ions_removidos  -- lista dos fragmentos descartados (SMILES)
        aviso                  -- alerta (string vazia se não houver) —
                                   preenchido quando um contra-íon metálico
                                   "não habitual" foi removido
    """
    resultado = {
        "smiles_usado": smiles,
        "multi_fragmento": False,
        "contra_ions_removidos": [],
        "aviso": "",
    }
    ...
    if mol is None:
        # SMILES inválido — calcular_descritores trata a invalidez à parte.
        return resultado

    resultado["multi_fragmento"] = True
    fragmentos_originais = list(Chem.MolToSmiles(mol).split("."))

    mol_parent_bruto = _LARGEST_FRAGMENT_CHOOSER.choose(mol)
    smiles_parent_bruto = Chem.MolToSmiles(mol_parent_bruto)

    fragmentos_restantes = list(fragmentos_originais)
    if smiles_parent_bruto in fragmentos_restantes:
        fragmentos_restantes.remove(smiles_parent_bruto)
    resultado["contra_ions_removidos"] = fragmentos_restantes

    mol_parent = _UNCHARGER.uncharge(mol_parent_bruto)
    try:
        Chem.SanitizeMol(mol_parent)
    except Exception:
        pass
    resultado["smiles_usado"] = Chem.MolToSmiles(mol_parent)

    metais_nao_habituais = set()
    for frag_smiles in fragmentos_restantes:
        frag_mol = Chem.MolFromSmiles(frag_smiles, sanitize=False)
        if frag_mol is None:
            continue
        for atomo in frag_mol.GetAtoms():
            simbolo = atomo.GetSymbol()
            if simbolo in SIMBOLOS_METALICOS and simbolo not in METAIS_CONTRAION_HABITUAL:
                metais_nao_habituais.add(simbolo)

    if metais_nao_habituais:
        resultado["aviso"] = (
            "Contra-íon(s) metálico(s) removido(s) na padronização ("
            + ", ".join(sorted(metais_nao_habituais))
            + ") — confira manualmente se a espécie farmacologicamente ativa não é "
            "o próprio metal (ex.: sais de zinco, lítio, ferro...), caso em que a "
            "predição de PK abaixo (feita sobre o fragmento orgânico remanescente) "
            "não é apropriada."
        )
...
def calcular_descritores(smiles: str, nome: str = "") -> dict:
    ...
    Se o SMILES tiver múltiplos fragmentos ('.'), a molécula é primeiro
    padronizada via padronizar_smiles() (maior fragmento + neutralização) —
    os descritores abaixo são calculados sobre esse SMILES padronizado, não
    sobre o original. O SMILES original de entrada, o SMILES efetivamente
    usado, e eventuais avisos de padronização ficam registrados nos campos
    'SMILES', 'SMILES_Usado', 'Multi_Fragmento', 'Contra_Ions_Removidos' e
    'Aviso_Padronizacao'.

    Retorna um dicionário com os valores, ou None nos campos numéricos se o
    SMILES (padronizado) for inválido.
    """
    padronizacao = padronizar_smiles(smiles)
    smiles_usado = padronizacao["smiles_usado"]

    base = {
        "Nome": nome,
        "SMILES": smiles,
        "SMILES_Usado": smiles_usado,
        "Multi_Fragmento": padronizacao["multi_fragmento"],
        "Contra_Ions_Removidos": "; ".join(padronizacao["contra_ions_removidos"]),
        "Aviso_Padronizacao": padronizacao["aviso"],
    }

    mol = Chem.MolFromSmiles(smiles_usado)
...
def main():
    ...
    try:
        ...
        #
        # Usa-se 'SMILES_Usado' (já padronizado por padronizar_smiles(), com
        # fragmentos desconectados removidos) em vez do SMILES original —
        # o Deep-PK rejeita SMILES com '.' (sais/pares iônicos).
        pares_validos = [
            (idx, desc["SMILES_Usado"])
            for idx, desc in enumerate(descritores_druglikeness)
            if desc["Valido"]
        ]
        ...
        n_multi_fragmento = sum(1 for r in descritores_druglikeness if r.get("Multi_Fragmento"))
        if n_multi_fragmento:
            print(
                f"[INFO] {n_multi_fragmento} SMILES com múltiplos fragmentos ('.') foram "
                "padronizados automaticamente (maior fragmento + neutralização) antes da "
                "análise local e do envio ao Deep-PK:"
            )
            for r in descritores_druglikeness:
                if not r.get("Multi_Fragmento"):
                    continue
                linha = f"    - {r['Nome']}: '{r['SMILES']}' -> '{r['SMILES_Usado']}'"
                if r.get("Aviso_Padronizacao"):
                    linha += f"\n      [AVISO] {r['Aviso_Padronizacao']}"
                print(linha)
